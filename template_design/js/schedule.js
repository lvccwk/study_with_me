...
async function getConfirmedSchedule() {
	...
	if (!otherUserId) {
		let res = await fetch(
			`/admin/schedule/confirm/${session.type}/${session.id}/${day.innerHTML}/`
		)
		return await res.json()
	} else {
		let result = await getOtherUser(otherUserId)
		let res = await fetch(
			`/admin/schedule/confirm/${result.type}/${otherUserId}/${day.innerHTML}`
		)
		return await res.json()
	}
}
...
export async function createScheduleTable(htmlId) {
	...
	hoursOfTheDay.forEach((hr) => {
		...
		const grid = $(`<div class="schedule-row" id="booking-${hr.id}"></div>`)

		const time = $(`<div class="col-1 schedule-box time">${hr.text}</div>`)

		const student = $(
			`<div class="col-2 schedule-box student"><span> </span></div>`
		)
...
async function getPendingSchedule() {
	let session = await getSession()
	let res = await fetch(`/admin/schedule/pending/${session.id}`)
	return await res.json()
}
...
async function scheduleInputSubmit() {
	...
	input.addEventListener('submit', async function (event) {
		...
		const res = await fetch(`/admin/add/${session.id}`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
